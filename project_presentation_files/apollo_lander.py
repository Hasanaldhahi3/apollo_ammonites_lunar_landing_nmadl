...
class ApolloLander(gym.envs.box2d.LunarLander):
    ...
    def step(self, action):
        ...
        # print("Obstacle penalty: " + str(-100 * (distance_to_obstacle <= (self.obstacle_params[2] + LANDER_RADIUS / SCALE))))
...
class ImaginativeApolloLander(gym.envs.box2d.LunarLander):
    ...
    """
    def reset(self, pos_x=0, pos_y=None, v_x=0, v_y=0, angle=0, angular_v=0, leg1contact=0, leg2contact=0):
        self._destroy()
        self.world.contactListener_keepref = ContactDetector(self)
        self.world.contactListener = self.world.contactListener_keepref
        self.game_over = False
        ...
        if pos_y is None:
            ...
            initial_y = pos_y*VIEWPORT_H / SCALE / 2 + (self.helipad_y + LEG_DOWN / SCALE)
        
        self.lander = self.world.CreateDynamicBody(
            position=(pos_x * VIEWPORT_W / SCALE / 2 + VIEWPORT_W / SCALE / 2, initial_y),
            angle=angle,
            linearVelocity=(v_x / ((VIEWPORT_W / SCALE / 2) / FPS), v_y/((VIEWPORT_H / SCALE / 2) / FPS)),
            angularVelocity=angular_v/(20.0 / FPS),
            fixtures=fixtureDef(
                shape=polygonShape(
                    vertices=[(x / SCALE, y / SCALE) for x, y in LANDER_POLY]
                ),
        ...
        for i in [-1, +1]:
            leg = self.world.CreateDynamicBody(
                position=(pos_x * VIEWPORT_W / SCALE / 2 + VIEWPORT_W / SCALE / 2 - i * LEG_AWAY / SCALE, initial_y),
                angle=(i * 0.05),
                linearVelocity=(v_x / ((VIEWPORT_W / SCALE / 2) / FPS), v_y/((VIEWPORT_H / SCALE / 2) / FPS)),
                angularVelocity=angular_v/(20.0 / FPS),
                fixtures=fixtureDef(
        ...
        self.legs[0].ground_contact = bool(leg1contact)
        self.legs[1].ground_contact = bool(leg2contact)
        
        self.drawlist = [self.lander] + self.legs

        #return self.step(np.array([0, 0]) if self.continuous else 0)[0]
        
class ApolloLanderModded(gym.envs.box2d.LunarLander):
    """
        We modify LunarLander class to add an obstacle and modify the reward
        shaping functions
    """
    ...
    def step(self, action):
        ...
        reward -= s_power * 0.03
        # Here we penalize for being in the obstacle's circle
        reward = reward - 1 * (distance_to_obstacle <= (self.obstacle_params[2] + LANDER_RADIUS / SCALE)) # Lander as circle
